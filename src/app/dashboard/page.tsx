...
function DashboardInner() {
 ...
 const fetchAllDatabases = useCallback(async (silent = false) => {
  ...
  // ★ ダッシュボードのレイアウトデータがまだ読み込めていない間は、
  //     デフォルトのスコアカードなどを表示せず、ローディング画面を表示する
  if (!dbLoaded) {
    return (
              ...
              <>
                <section>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <button onClick={()=>addWidget('ranking-card','rectangle')} className="flex items-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium hover:border-indigo-400 hover:text-indigo-600 hover:shadow-sm transition-all group">
                      <span className="p-1.5 bg-pink-50 rounded-lg text-pink-500 group-hover:bg-pink-100 transition-colors"><Icons.ChartBar className="w-4 h-4" /></span>
                      ランキングカードを追加
                    </button>
                  </div>
                </section>
          ...
          {loadingAll && dbLoaded && cacheLoaded && Object.keys(cacheStore).length === 0 && (
            <div className="absolute inset-0 z-[100] bg-white/70 backdrop-blur-sm flex flex-col items-center justify-center gap-4">
              ...
              <div className="flex flex-col items-center gap-1">
                <h3 className="text-lg font-bold text-slate-700">データを取得しています...</h3>
                <p className="text-sm text-slate-500">Notion / WordPress から最新の情報を読み込み中</p>
              </div>
            </div>
          )}
